...
void maxHD(struct tnode *root,int *maxhd){
    if (root == NULL){return;}
    if(absolute(root->hd)>absolute(*maxhd)){*maxhd = root->hd;}
...
struct tnode *initTreeFromPreAndIn(int inOrder[], int preOrder[], int front, int rear, int *preIndex){
    ...
    for (int i = front; i <= rear; i++){
        if (inOrder[i] == newnode->data){r = i;break;}
    }
    ...
    if (root != NULL){
        if (root->data == key2){
            if (root->left != NULL && root->right != NULL){
                if (root->hd > 0){insert(root->right, key1);}
    ...
    if ((*root)->data == key){
        ...
        if((*root)->left==NULL && (*root)->right ==NULL){
            *root=NULL;return;
            ...
            if ((*root)->hd >= 0){delete (&((*root)->left), &replacementKey, &flag);}
...
{   struct tnode *root =NULL;
    ...
    while(1){
        ...
        if(ch==1){
            ...
            root = treeFromPreAndIn(preOrder, inOrder, size);
            updateTree(root);
        }
        else if(ch==2){
            int key1,key2;
