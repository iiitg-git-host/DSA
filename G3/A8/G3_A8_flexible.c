...
#define max_photo_rating 20
#define max_rating_size 10
#define max_heap_rating_size (max_rating_size / 2) + 1
#define valid_rating_margin 3

typedef struct photo
{
    int identifier;

    int ratings[max_rating_size];
    int rating_count;
    int primordial_rating;
    float average_rating;

    int max_heap_rating[max_heap_rating_size];
    int max_heap_size;

    int min_heap_rating[max_heap_rating_size];
    int min_heap_size;
} photo;
...
int check_rating(int rating, int median)
{
    if (rating >= 0 && rating >= median - valid_rating_margin && rating <= max_photo_rating && rating <= median + valid_rating_margin)
    {
        return 1;
    }
    else
...
void add_rating(photo photos[], int size, int idx)
{
    ...
    else
    {
        ...
        {
            if (photos[idx].rating_count < max_rating_size)
            {
...
int main()
{
    photo photosdsliiitg[max_photo_size];
    int photo_count = 0;
    while (1)
    {
        printf(" ####################################### \n");
